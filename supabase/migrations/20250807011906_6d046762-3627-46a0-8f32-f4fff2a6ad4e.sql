-- First, temporarily drop the foreign key constraints
ALTER TABLE public.pyg_annual 
DROP CONSTRAINT IF EXISTS pyg_annual_concepto_codigo_fkey;

ALTER TABLE public.allocation_rules 
DROP CONSTRAINT IF EXISTS allocation_rules_concepto_codigo_fkey;

-- Update existing data to match new grupo values
UPDATE public.catalog_pyg_concepts SET grupo = 'Ingresos' WHERE grupo IN ('INGRESOS');
UPDATE public.catalog_pyg_concepts SET grupo = 'Costes' WHERE grupo IN ('COSTES');
UPDATE public.catalog_pyg_concepts SET grupo = 'Gastos' WHERE grupo IN ('GASTOS_OPERATIVOS', 'AMORTIZACIONES', 'OTROS_OPERATIVOS');
UPDATE public.catalog_pyg_concepts SET grupo = 'Financieros' WHERE grupo IN ('FINANCIEROS');
UPDATE public.catalog_pyg_concepts SET grupo = 'No Operativo' WHERE grupo IN ('EXTRAORDINARIOS');
UPDATE public.catalog_pyg_concepts SET grupo = 'Impuestos' WHERE grupo IN ('IMPUESTOS');

-- Now update the catalog_pyg_concepts table structure
...
-- Update existing records to match new specification and insert any missing ones
...
ALTER TABLE public.pyg_annual 
ADD CONSTRAINT pyg_annual_concepto_codigo_fkey 
FOREIGN KEY (concepto_codigo) REFERENCES public.catalog_pyg_concepts(concepto_codigo);

ALTER TABLE public.allocation_rules 
ADD CONSTRAINT allocation_rules_concepto_codigo_fkey 
FOREIGN KEY (concepto_codigo) REFERENCES public.catalog_pyg_concepts(concepto_codigo);