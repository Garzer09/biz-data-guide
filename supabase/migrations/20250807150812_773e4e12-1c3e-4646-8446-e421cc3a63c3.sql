...
SELECT
  company_id,
  AVG((principal + intereses) / NULLIF(flujo_operativo,0)) AS dscr_promedio,
  MIN((principal + intereses) / NULLIF(flujo_operativo,0)) AS dscr_minimo,
  SUM(CASE WHEN (principal + intereses) / NULLIF(flujo_operativo,0) < 1 THEN 1 ELSE 0 END) 
    AS meses_en_riesgo,
  SUM(principal + intereses) FILTER (WHERE periodo LIKE LEFT(MAX(periodo) OVER (),4)||'-%') 
    AS servicio_anual
FROM public.vw_debt_service_detail
GROUP BY company_id;